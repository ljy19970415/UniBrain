...
from ..augmentations.noise_augmentations import augment_blank_square_noise, augment_gaussian_blur, \
...
class GaussianBlurTransform(AbstractTransform):
    ...
    def __call__(self, data):
        if np.random.uniform() < self.p_per_sample:
            data = augment_gaussian_blur(data, self.blur_sigma, self.different_sigma_per_channel, self.p_per_channel)
        return data
